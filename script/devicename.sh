...
exp_internal 1
# 디버깅시 주석 제거
#log_file devicename_debug.log
# 모든 명령어 전 대기 시간
set timeout 5

#로그파일 초기화
file delete "devicename.log"
...
# Serial과 MAC 주소 추출 및 hostname 설정을 처리하는 프로시저
proc process_device {ip user pass new_ip subnet gw sz hostname} {
    ...
    # Hostname 변경
    puts "($ip의 hostname을 \"$hostname\"로 변경합니다.)"
	expect -re "rkscli"
	...
	puts "($ip의 hostname을 \"$hostname\"로 변경하였습니다. 다음 장비로 이동합니다.)"
}
...
while {[gets $fp line] != -1} {
	...
	# 메세지출력
    puts "처리 중: $ip의 hostname을 \"$hostname\"로 변경하는중..."
	...
	# 3) 새로운 패스워드 입력하라고 나올 때 패스워드 ruckus12#$ 강제 적용
            -re "New password" {
                send "ruckus12#$\r"
                expect -re "Confirm password"
                send "ruckus12#$\r"
...
set result_fp [open "devicename_result.csv" w]

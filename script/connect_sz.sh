...
#exp_internal 1
# 디버깅시 주석 제거
#log_file conect_sz_debug.log
# 모든 명령어 전 대기 시간
set timeout 5

#로그파일 초기화
file delete "connect_sz.log"

# PHP에서 업로드된 CSV 파일 경로 받기
set filename [lindex $argv 0]

# Serial과 MAC 주소 추출 및 SZ IP 설정을 처리하는 프로시저
proc process_device {ip user pass new_ip subnet gw sz hostname} {
    ...
    puts "($ip의 sz IP을 $sz로 변경하였습니다. 다음 장비로 이동합니다.)"
}
...
while {[gets $fp line] != -1} {
	...
	# 메세지출력
    puts "처리 중: $ip의 sz IP를 $sz로 설정하는중..."
...
set result_fp [open "connect_sz_result.csv" w]

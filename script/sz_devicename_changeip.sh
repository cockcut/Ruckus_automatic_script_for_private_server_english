...
#exp_internal 1
# 디버깅시 주석 제거
#log_file sz_devicename_changeip_debug.log
# 모든 명령어 전 대기 시간
set timeout 5

#로그파일 초기화
file delete "sz_devicename_changeip.log"
...
# Serial과 MAC 주소 추출 및 SZ, hostname, IP 설정을 처리하는 프로시저
proc process_device {ip user pass new_ip subnet gw sz hostname} {
    ...
    puts "($ip의 SZ IP, Hostname, IP를 변경하였습니다. 다음 장비로 이동합니다.)"
}
...
while {[gets $fp line] != -1} {
	...
	# 메세지출력
    puts "처리 중: $ip의 SZ($sz), Hostname($hostname), 새로운 고정IP($new_ip)를 설정하는 중..."
			...
			-re "Login incorrect" {
				...
				# 2-1-2) sp-admin로 시도하였는데 Newpassword가 나오면 초기화된 AP이므로 sp-admin -> ruckus12#$ 강제 적용
						-re "New password" {
							...
							send "ruckus12#$\r"
							# Confirm password 후에 Please login을 다시 요청
							expect -re "Please login"
							send "$user\r"
							expect -re "password"
							#패스워드 ruckus12#$ 강제 적용
...
set result_fp [open "sz_devicename_changeip_result.csv" w]
